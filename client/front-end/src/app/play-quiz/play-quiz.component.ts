...
export class PlayQuizComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.router.paramMap.subscribe((params: ParamMap) => {
      ...
      this.creationService.GetQuiz(this.id).subscribe((res) => {
        ...
        this.quizData['quizId'] = res['id']
        this.quizData['passcode'] = res['passcode']
        this.response = res
        if (res['passcode']) {
          if (this.passcode) {
            if (this.passcode === res['passcode']) {
              canProceed = true
            }
          } else {
  ...
  checkPassword(value) {
    if (this.quizData['passcode'] === value) {
      this.showPassword = false
      this.proceed()
    } else {
      this.errorMessage = "Password incorrect try again !!.. "
    }
  }
  setName(value) {
  ...
  proceed() {
    ...
    res.forEach((element, index) => {
      ...
      questionGroup.push(this.createOptionsGroup(element.options, element.answers))
    });
  ...
  onSubmit(form) {
    ...
    for (let ques = 0; ques < formValue.length; ques++) {
      ...
      if (this.questions[ques]['answers'].length > 1) {
        for (let opt = 0; opt < formValue[ques]['proposedAnswer'].length; opt++) {
          if (formValue[ques]['proposedAnswer'][opt]) {
            ...
            answeredAnswer.push(this.questions[ques]['options'][opt])
          }
        }
      } else {
      ...
      if (this.isSubset(this.questions[ques]['answers'], answeredAnswer)) {
        total += 1
        correctAnswer = true
    ...
    this.quizData['numberOfQuestions'] = `${formValue.length}`
    console.log(this.quizData)
    this.creationService.submitQuiz(this.quizData).subscribe(res => {
      console.log(res['message'])
      this.showLoader = false
      this.navRouter.navigate(['feedback', this.quizData['quizId']])
    },
      err => {
        this.showLoader = false
  ...
  isSubset(answer: any[], options: any[]) {
    ...
    options.forEach(element => {
      if (answer.find((ans) => ans.answer == element.option)) {
        count++
      }
    })
  ...
  isRadio(index) {
    return this.questions[index]['answers'].length === 1 ? true : false
  }
